...
function Notice({ children, tone = 'info', onClose }) {
  return (
    <div className={`auth-notice ${tone}`} role="alert">
      <span>
        {tone === 'success' ? (
...
function AuthShell({ children, mode, onMode }) {
  return (
...
function Login({ onMode, onForgot }) {
  ...
  function submit(event) {
    ...
    if (!Object.keys(next).length) {
      sessionStorage.setItem('nexora-auth', JSON.stringify({ email }))
...
function ResetModal({ onClose }) {
  ...
  function reset(e) {
    e.preventDefault()
    if (code !== demoCode || password.length < 8) {
      setError(
        code !== demoCode
      ...
      >
        ...
        {step === 'done' ? (
          ...
          <>
            ...
            <form
              className="auth-form"
              onSubmit={step === 'email' ? send : reset}
            >
              {step === 'email' ? (
                <Field
                  label="Email address"
...
export default function AuthPage() {
  ...
  const heading = useMemo(() => mode, [mode])
  return (
    <AuthShell mode={heading} onMode={setMode}>
      {mode === 'login' ? (
        <Login onMode={setMode} onForgot={() => setForgot(true)} />
      ) : (
        <Register onMode={setMode} />
      )}
